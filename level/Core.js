...
function fogRGB(color, distance) {
	...
	if (Map.fog) {
		...
		if (distance > Map.fog.max) {
			...
			var m = (distance - Map.fog.min) / (Map.fog.max - Map.fog.min);
			m = Math.min(Math.max(m, 0), 1);
			color[0] = Math.floor((color[0] * (1 - m)) + (fogColor[0] * m));
			color[1] = Math.floor((color[1] * (1 - m)) + (fogColor[1] * m));
			color[2] = Math.floor((color[2] * (1 - m)) + (fogColor[2] * m));
		}
	}
	return color
}
...
function drawVertical(x, perpWallDist, side, tile, texX, z=.5, sh=1) {
	...
	var lineHeight = (h / perpWallDist)*sh;
	var drawStart = (h * Cam.p - lineHeight * z);
	var drawEnd = (lineHeight * (1-z) + h * Cam.p);
...
function drawObjects() {
	...
	for (let i = 0; i < Map.object.length; i++) {
		...
		if (obj.tile > 0) {
			...
			var spriteW = Math.abs(Math.floor(h / transformY)) * sprite.sw;
			...
			for (let stripe = drawStartX; stripe < drawEndX; stripe++) {
				if (transformY > 0 & stripe > 0 & stripe < w & transformY < ZBuffer[stripe]) {
					...
					drawVertical(stripe, transformY, 0, tile, texX, obj.z, .5, sprite.sh)
				}
			}
		}
	}
...
function Raycast() {
	...
	for (let x = 0; x < w; x++) {
		...
		if (level.blocks) { tile = level.blocks[tile-1].texture[0] }
		drawVertical(x, perpWallDist, side, tile, texX);
		ZBuffer[x] = perpWallDist;
		if(x==Math.floor(w/2)){
			Cam.target.idx = getTile(mapX, mapY).idx
			Cam.target.type = getTile(mapX, mapY).type
			Cam.target.dist = perpWallDist
		}
	}
	drawObjects();
}
...
export function playSound(soundID, gain=1, pan=0){
...
// Control
export var Keys = {}
export var Mouse = { x: 0, y: 0, z: 0, b0: 0, b1: 0, b2: 0 }
...
document.addEventListener('mousedown', (event) => {
	Mouse["b"+event.button]=1;
}, false);
document.addEventListener('mouseup', (event) => {
	Mouse["b"+event.button]=0;
}, false);
...
function moveCamera(DTime) {
	var walkSpeed = Cam.walkSpeed*DTime/1000 || 0;
	var turnSpeed = Cam.turnSpeed*DTime/1000 || 0;
	var pitchSpeed = Cam.pitchSpeed*DTime/1000 || 0;
	...
	Cam.p = Math.max(-0.5, Math.min(Cam.p, 1.5))
...
//Tools
export function changeMap(map, x, y){
    RUN = false;
    MapIdx = map;
    Map = level.map[map];
    Cam.x = x;
...
function refreshDebug(DTime) {
	...
	if (frame % 8 == 0) {
		...
		debug.innerHTML = " fps:" + fps + ", x:" + Cam.x + ", y:" + Cam.y;
		bigD = 0;
	}
};
